...
def run():
    data_dir = os.path.join(os.getcwd(), 'drone_data', 'jpeg')
    data_dirs = {}
    for i in ['validation', 'train', 'test']:
        data_dirs[i] = os.path.join(data_dir, i)
        tf.gfile.MakeDirs(data_dirs[i])

    output_dir = os.path.join(os.getcwd(), 'drone_data', 'tfrecord')
    output_dirs = {}
    for i in ['validation', 'train', 'test']:
