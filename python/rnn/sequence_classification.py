...
# No use 'context feature' in SequenceExample here.
####
NUM_HIDDEN = 5
NUM_CLASS = 3
...
def loss(logits, labels):
    ...
    cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits, labels=labels_flat, name='cross_entropy')
...
def error(predictions, labels):
    labels_flat = tf.reshape(labels, [-1], name='labels_flat')
    errors = tf.not_equal(tf.argmax(predictions, 1), labels_flat, name='errors')
...
def eval(model_dir):
    ...
    with tf.Graph().as_default():
        ...
        avg_error = error(predictions, labels)
...
if __name__ == '__main__':
    model_dir = os.path.join(os.getcwd(), 'cls_models')
