...
def _nn_layer(input_tensor, input_dim, output_dim, layer_name, act=tf.nn.relu):
    ...
    with tf.name_scope(layer_name):
        ...
        activations = act(preactivations, 'activations') if act != None else tf.identity(preactivations, 
                                                                                         name='activations')
        tf.histogram_summary(layer_name + '/activations', activations)
        return activations

def _conv_layer(input_tensor, psize, in_channels, out_channels, layer_name, act=tf.nn.relu):
    # Adding a name scope ensures logical grouping of the layers in the graph.
    with tf.name_scope(layer_name):
        with tf.name_scope('weights'):
            weights = _weight_variable([psize, psize, in_channels, out_channels]) 
        with tf.name_scope('biases'):
...
def _maxpool_layer(input_tensor, ksize, layer_name):
    # Adding a name scope ensures logical grouping of the layers in the graph.
    with tf.name_scope(layer_name):
        activations = tf.nn.max_pool(input_tensor, ksize=[1, ksize, ksize, 1],
...
def _build_model(input_tensor, dropout_keep_rate):
    ...
    layer_1 = _conv_layer(x_image, 5, 1, NUM_CONV_1, 'layer1') # outputs: NUM_CONV_1 * NUM_INPUT_XS * NUM_INPUT_YS
    
    # 2x2 Max Pooling Layer
    xs = NUM_INPUT_XS // 2
    ys = NUM_INPUT_YS // 2
    layer_2 = _maxpool_layer(layer_1, 2, 'layer2') # outputs: NUM_CONV_1 * xs * ys
    
    # Convolutional Layer (5x5 patch size, 32 input channel, 64 output channels)
    layer_3 = _conv_layer(layer_2, 5, NUM_CONV_1, NUM_CONV_2, 'layer3') # outputs: NUM_CONV_2 * xs * ys
    
    # 2x2 Max Pooling Layer
    xs = xs // 2
    ys = ys // 2
    layer_4 = _maxpool_layer(layer_3, 2, 'layer4') # outputs: NUM_CONV_2 * xs * ys
    ...
    layer_5_drop = tf.nn.dropout(layer_5, dropout_keep_rate, name='dropout_layer')
    
    # Output Layer
    output_layer = _nn_layer(layer_5_drop, NUM_FC_1, NUM_OUTPUT, 'output_layer', act=None)
    return output_layer
...
def get_placeholders():
    x = tf.placeholder(tf.float32, [None, NUM_INPUT_XS * NUM_INPUT_YS], name='input_data')
